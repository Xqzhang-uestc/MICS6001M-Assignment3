...
def build_lstm_model(n_input, n_output, activation, learning_rate=0.001):
    """
    Build an LSTM model.
    ...
    model = Sequential([
        ...
        LSTM(100, activation=activation, return_sequences=False),
...
def train_lstm(data, n_input, n_output, file, activations, epochs_list, learning_rate=0.001, data_name='data'):
    """
    ...
    for activation in activations:
        for epochs in epochs_list:
            print(f"Training LSTM - Activation: {activation}, Epochs: {epochs}")
            model = build_lstm_model(n_input, n_output, activation, learning_rate=learning_rate)
            
            # Train the model
            history = model.fit(X_train, y_train, epochs=epochs, batch_size=16, verbose=0, validation_data=(X_test, y_test))
            
            # Plot loss curve
            plot_loss_curve(history, structure="LSTM", activation=activation, file_prefix='lstm', epochs=epochs, data_name=data_name, data_amplitude=data_amplitude)
            ...
            # Flatten predictions and true values to ensure shape consistency
            ...
            # Plot predictions
            plot_predictions(y_train, y_test, y_train_pred, y_test_pred, file_prefix=f'lstm_{data_name}_{activation}_{epochs}')
            ...
            with open(file, 'a') as f:
                f.write(f"LSTM - Activation: {activation}, Epochs: {epochs}, Test Loss (MSE): {loss}, Test MAE: {mae}\n")
...
if __name__ == "__main__":
    ...
    # train_lstm(white_noise, n_input=16, n_output=1, file='white_noise_results.txt', activations=activations, epochs_list=epochs_list, learning_rate=learning_rate, data_name='white_noise')
    # train_lstm(random_walk, n_input=16, n_output=1, file='random_walk_results.txt', activations=activations, epochs_list=epochs_list, learning_rate=learning_rate, data_name='random_walk')
    train_lstm(arma_process, n_input=16, n_output=1, file='arma_process_results.txt', activations=activations, epochs_list=epochs_list, learning_rate=learning_rate, data_name='arma_process')