...
noise_amplitude = 0.0005  # Adjust this to control signal-to-noise ratio
noise = np.random.normal(0, 1, len(data)) * noise_amplitude
noisy_data = data + noise

# Perform statistical tests on the generated data
draw_plots(noisy_data, lags=40, name="noisy_equal_difference_series")  # Draw plots for the generated data

perform_statistical_tests(noisy_data, lags=40)  # Perform statistical tests on the generated data

# Reshape the data for supervised learning
n_input = 4  # Input vector size
n_output = 1  # Output vector size
X, y = reshape_to_supervised(noisy_data, n_input, n_output)
...
model_structure = "64-32-16"  # Model structure
